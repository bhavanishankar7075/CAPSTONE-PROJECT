...
export const createComment = async (req, res) => {
  try {
    ...
    const comment = new Comment({ text, userId: req.user.id, videoId: req.params.videoId });
    await comment.save();
...
export const updateComment = async (req, res) => {
  try {
    ...
    if (String(comment.userId) !== req.user.id) return res.status(403).json({ message: "Not allowed" });
